...
const VoucherTemplate: React.FC<{ booking: Booking, agentName?: string }> = ({ booking, agentName }) => {
            ...
            <div className="p-12">
                ...
                {agentName && (
                    <div className="mb-12">
                        ...
                        <div className="h-0.5 w-full bg-[#006D77] mb-4 opacity-30"></div>
                        <p className="text-xl font-black text-gray-800">{agentName}</p>
                    </div>
                )}
                ...
                <footer className="flex justify-between items-start">
                    <div className="flex-1"><h4 className="text-sm font-black text-gray-800 mb-4 uppercase tracking-widest">Important Notes:</h4><ul className="space-y-3">{['Please present this voucher upon check-in.', 'This booking is non-refundable unless otherwise stated.', 'For assistance, contact us at support@umrahhotels.com.'].map((note, i) => (<li key={i} className="flex gap-3 text-xs font-bold text-gray-400 leading-relaxed"><span className="text-[#006D77]">•</span>{note}</li>))}</ul></div>
    ...
    return (
        <>
            ...
            <div className="animate-in fade-in duration-500 voucher-container">
                <VoucherTemplate booking={booking} agentName={agent?.agencyName} />
            </div>
